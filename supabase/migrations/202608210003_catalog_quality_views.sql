...
select
  ...
  greatest(0, 27 - count(hotel.id) filter (where hotel.active and hotel.review_status = 'verified' and hotel.recommendation_ready)) as recommendation_ready_shortfall
...
select destination.id as destination_id, matrix.price_tier, matrix.vibe_tier,
  ...
  3 as target_count,
  greatest(0, 3 - count(classified.destination_id)) as shortfall
from public.travel_destinations destination
cross join matrix
