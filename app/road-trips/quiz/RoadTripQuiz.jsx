...
export function RoadTripQuiz() {
  ...
  const selectedJourney = roadTripDiscovery.find((item) => item.id === journeyId) || null;
  ...
  const ready = question.id === "budget" ? Number(answers.budget) >= 500 : question.id === "origin" ? Boolean(answers.originDetails?.city && answers.originDetails?.countryCode && Number.isFinite(answers.originDetails?.latitude) && Number.isFinite(answers.originDetails?.longitude)) : Boolean(selected && (selected !== "Custom distance" || Number(answers.customDistance) >= 50));

  function choose(option) { setAnswers((current) => ({ ...current, [question.id]: option })); }
  function next() {
    if (!ready) return;
    if (step < questions.length - 1) { setStep((current) => current + 1); window.scrollTo({ top: 0, behavior: "smooth" }); return; }
    const payload = { ...answers, ...(answers.style ? { kind: answers.style === "Scenic and slow" ? "Slow scenic journey" : answers.style === "Maximum exploring" ? "Adventure route" : "Food and culture" } : {}), requestedRouteId: selectedJourney?.id || null, version: 2, createdAt: new Date().toISOString() };
  ...
  return <section className="min-h-[calc(100svh-8rem)] px-5 py-10 sm:px-8 sm:py-16">
    <div className="mx-auto max-w-[1320px]">
      <div className="flex items-center justify-between border-b border-black/12 pb-5 text-[10px] uppercase tracking-[0.18em] text-black/45"><button type="button" onClick={() => step ? setStep(step - 1) : router.push("/road-trips")} className="text-black/65">← Back</button><span>{selectedJourney ? selectedJourney.name : "Road discovery"} · {String(step + 1).padStart(2, "0")} / {String(questions.length).padStart(2, "0")}</span></div>
      <div className="pt-12 sm:pt-16">
        ...
        </div> : question.id === "origin" ? <div className="mt-14 max-w-2xl sm:mt-20"><p className="text-[10px] uppercase tracking-[0.18em] text-black/45">Leaving from</p><StartingLocationField id="road-origin" value={answers.originDetails} onChange={(originDetails) => setAnswers((current) => ({ ...current, originDetails }))} /><p className="mt-5 text-xs text-black/42">Used for drive distance, flight access when needed, and the complete trip budget.</p></div> : <div className={`mt-12 sm:mt-16 ${question.id === "landscape" ? "grid gap-3 sm:grid-cols-2 lg:grid-cols-4" : "grid max-w-4xl gap-x-10 sm:grid-cols-2"}`}>
