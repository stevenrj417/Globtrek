  ...
  return <>
    ...
    <section className="bg-[#f4f1eb] px-6 py-20 sm:px-10 sm:py-28">
      <div className="mx-auto grid max-w-[1320px] gap-12 lg:grid-cols-[1.15fr_.85fr] lg:gap-24">
        ...
        <div className="self-end border-t border-black/15 pt-6"><p className="text-sm leading-7 text-black/58">This route reflects your pull toward {String(answers.landscape).toLowerCase()}, a {String(answers.kind).toLowerCase()}, and {String(answers.driving).toLowerCase()}.</p><div className="mt-8 grid grid-cols-2 gap-6 text-sm"><div><p className="text-[9px] uppercase tracking-[0.15em] text-black/40">Planning estimate</p><p className="mt-2 font-serif text-2xl">{money(route.estimate.low)}–{money(route.estimate.high)}</p></div><div><p className="text-[9px] uppercase tracking-[0.15em] text-black/40">Your budget</p><p className="mt-2 font-serif text-2xl">{money(Number(answers.budget))}</p></div></div><p className={`mt-6 text-xs ${route.compatibility.level === "poor" ? "text-[#8a4c37]" : "text-black/48"}`}>{route.compatibility.level === "excellent" ? "Estimated to fit within your total budget." : route.compatibility.level === "acceptable" ? "Close to your budget; the lower end is the practical target." : "Closest grounded route, but currently estimated above budget. It is not presented as within budget."} Estimates include stays, food, experiences, and road costs; they are not live prices.</p></div>
      </div>
      <div className="mx-auto mt-20 max-w-[1500px] space-y-16 sm:mt-28 sm:space-y-24">{route.stops.map((stop, index) => <article key={stop.id} className={`grid gap-7 lg:grid-cols-2 lg:items-center lg:gap-16 ${index % 2 ? "lg:[&>div:first-child]:order-2" : ""}`}><div className="relative aspect-[4/3] overflow-hidden bg-[#d7d0c4]"><Image src={stop.image} alt={`${stop.city}, ${stop.country}`} fill sizes="(min-width:1024px) 55vw, 100vw" className="object-cover" /></div><div className="max-w-xl"><p className="text-[10px] uppercase tracking-[0.18em] text-[#8a6b36]">Stop {String(index + 1).padStart(2, "0")}</p><h3 className="mt-4 font-serif text-[clamp(3.2rem,6vw,6rem)] leading-[.86] tracking-[-0.05em]">{stop.city}</h3><p className="mt-5 text-sm text-black/50">{stop.country} · {stop.style}</p></div></article>)}</div>
    </section>
