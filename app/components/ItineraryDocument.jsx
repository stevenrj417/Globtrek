...
export function ItineraryDocument({ trip, quiz, previewDays = 3, fullTripHref = null }) {
  ...
  return <>
    ...
    {places.length ? <section className="bg-[#ded8cf] px-6 py-20 sm:px-12 sm:py-28" aria-labelledby="places-heading"><div className="mx-auto max-w-[1240px]"><p className="text-xs text-black/40">Places in your itinerary</p><h2 id="places-heading" className="mt-4 font-serif text-[clamp(2.7rem,5vw,5rem)] leading-[.95] tracking-[-0.045em]">Dining and plans.</h2><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{places.map((place, index) => { const image = displayableImage(place); const credits = place.imageAttribution || []; return <a key={`${place.kind}-${place.name}`} href={placeProviderUrl(place, trip)} target="_blank" rel={place.bookingUrl ? "noopener sponsored" : "noopener"} className="group flex min-h-[240px] flex-col bg-[#f4f1eb]">{image ? <div className="relative aspect-[16/10] overflow-hidden"><Image src={image} alt={`${place.name} in ${trip.city}`} fill unoptimized={place.provider === "google_places"} sizes="(min-width:1024px) 33vw,(min-width:640px) 50vw,100vw" className="object-cover transition duration-500 group-hover:scale-[1.02]" />{credits.length ? <span className="absolute bottom-2 right-2 bg-black/60 px-2 py-1 text-[8px] text-white/90">Photo: {credits.map((credit) => credit.displayName || "Google contributor").join(", ")}</span> : null}</div> : null}<span className="flex flex-1 items-end justify-between gap-5 p-6"><span><span className="text-[10px] text-black/40">{place.kind} · Day {Math.min(index + 1, Math.max(days.length, 1))}</span><strong className="mt-2 block font-serif text-2xl font-normal">{place.name}</strong>{place.rating ? <span className="mt-2 block text-[10px] text-black/45">{place.rating} · {Number(place.reviewCount || 0).toLocaleString("en-US")} reviews</span> : null}{place.provider === "opentable" ? <span className="mt-2 block text-[9px] uppercase tracking-[0.14em] text-black/40">Powered by OpenTable</span> : null}</span><span className="flex items-center gap-2 text-xs font-medium">{place.action}<ArrowUpRight /></span></span></a>; })}</div><p className="mt-5 text-xs leading-5 text-black/45">Reservation and ticket availability are confirmed on the linked provider. No availability is implied here.</p></div></section> : null}
  </>;
}
