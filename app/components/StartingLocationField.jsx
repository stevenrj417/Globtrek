...
export function StartingLocationField({ value, onChange, id = "starting-location", placeholder = "City, airport, or code", centered = false }) {
  ...
  return <div className="relative"><input id={id} aria-label="Leaving from" aria-required="true" role="combobox" aria-expanded={open && locations.length > 0} aria-controls={`${id}-results`} autoComplete="off" value={query} onFocus={() => setOpen(true)} onBlur={() => window.setTimeout(() => setOpen(false), 150)} onChange={(event) => { setQuery(event.target.value); onChange(null); setOpen(true); }} placeholder={placeholder} className={`w-full border-0 border-b border-black/30 bg-transparent py-5 text-xl outline-none ring-0 placeholder:text-black/25 focus:border-black focus:outline-none focus:ring-0 ${centered ? "text-center" : ""}`} />{open && (locations.length || status) ? <div id={`${id}-results`} className="absolute inset-x-0 top-full z-40 max-h-80 overflow-auto border border-black/15 bg-[#fbfaf7] shadow-xl">{locations.map((location) => <button type="button" key={location.placeId} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(location)} className={`w-full border-b border-black/10 px-5 py-4 last:border-b-0 hover:bg-[#eeeae2] ${centered ? "grid justify-items-center gap-2 text-center" : "flex items-center justify-between text-left"}`}><span><strong className="block text-sm font-medium">{location.city}</strong><span className="mt-1 block text-[10px] uppercase tracking-[0.1em] text-black/45">{location.airportName || location.formattedAddress}{location.countryName ? ` · ${location.countryName}` : ""}</span></span>{location.airportCode ? <span className="text-xs font-semibold tracking-[0.14em]">{location.airportCode}</span> : null}</button>)}{status ? <p aria-live="polite" className={`px-5 py-4 text-xs text-black/45 ${centered ? "text-center" : ""}`}>{status}</p> : null}</div> : null}</div>;
}
