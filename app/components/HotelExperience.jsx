...
function HotelCard({ hotel, destination, trip, index, count, onChoose, onDetails, onPrevious, onNext, priority = false, proposalMode = false, selected = false, compact = false, dense = false }) {
  ...
  if (dense) return <article className="grid min-w-full snap-center gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-center lg:gap-12" aria-label={`${hotel.name}, hotel ${index + 1} of ${count}`}>
    <div className="flex min-h-[390px] flex-col justify-between py-2"><div><p className="text-[9px] uppercase tracking-[.18em] text-black/45">01 &nbsp;&nbsp; Your stay</p><button type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="mt-5 block text-left"><h3 className="font-serif text-[clamp(2.35rem,4vw,4.1rem)] leading-[.9] tracking-[-.045em]">{verifiedHotel.name}</h3></button><p className="mt-5 text-[10px] uppercase tracking-[.12em] text-black/52">{locationLabel(verifiedHotel, destination)}</p>{ratingLabel(verifiedHotel) ? <p className="mt-4 text-xs">★ &nbsp;{ratingLabel(verifiedHotel)}</p> : null}{verifiedHotel.amenities?.length ? <p className="mt-5 text-xs leading-6 text-black/55">{verifiedHotel.amenities.slice(0, 6).join(" · ")}</p> : null}<p className="mt-5 text-xs text-black/55">{profile.tripLength} nights &nbsp;·&nbsp; {dateLabel(trip.tripStart) && dateLabel(trip.tripEnd) ? `${dateLabel(trip.tripStart)} — ${dateLabel(trip.tripEnd)}` : "Dates flexible"}</p></div><div className="mt-8 flex flex-wrap items-center gap-5">{bookingUrl ? <a href={bookingUrl} target="_blank" rel="noopener sponsored" onClick={() => onChoose(verifiedHotel)} className="inline-flex min-h-11 items-center bg-black px-6 text-[9px] uppercase tracking-[.13em] text-white">Check rooms &nbsp;→</a> : <button type="button" onClick={() => onChoose(verifiedHotel)} className="inline-flex min-h-11 items-center bg-black px-6 text-[9px] uppercase tracking-[.13em] text-white">{selected ? "Selected ✓" : "Select stay →"}</button>}<button type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="border-b border-black/30 pb-1 text-[9px] uppercase tracking-[.12em]">View details</button></div></div>
    <div className="relative"><div className="relative aspect-[16/10] overflow-hidden bg-[#d8d2c8]"><PropertyImage photo={photos} hotel={verifiedHotel} destination={destination} priority={priority} loading={loading} fillContainer sizes="(min-width:1024px) 62vw,100vw" /><button type="button" onClick={() => onPrevious()} aria-label="Previous hotel" className="absolute left-3 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-[#f7f5f0]/92">←</button><button type="button" onClick={() => onNext()} aria-label="Next hotel" className="absolute right-3 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-[#f7f5f0]/92">→</button></div><div className="mt-1 grid h-24 grid-cols-3 gap-1">{photos.slice(1, 4).map((photo, photoIndex) => <button key={photo.photoUri || photoIndex} type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="relative overflow-hidden"><PropertyImage photo={photo} hotel={verifiedHotel} destination={destination} fillContainer sizes="20vw" /></button>)}</div><p className="absolute -right-1 -bottom-8 text-[9px] tracking-[.12em] text-black/48">{String(index + 1).padStart(2, "0")} / {String(count).padStart(2, "0")}</p></div>
  </article>;
