...
function PlannerHotelDrawer({ hotel, destination, trip, photos, onClose, onChoose }) {
  const fullMedia = useHotelMedia(hotel, 5);
  const gallery = fullMedia.photos.length > photos.length ? fullMedia.photos : photos;
  const closeRef = useRef(null);
  const bookingUrl = bookingPropertyUrl(hotel, trip);
  ...
  return <div className="fixed inset-0 z-[70] grid place-items-end bg-black/72 p-3 sm:place-items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="hotel-detail-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="relative grid max-h-[94svh] w-full max-w-[1320px] overflow-y-auto rounded-lg bg-[#f7f5f0] lg:grid-cols-[1.55fr_.85fr]">
      <button ref={closeRef} type="button" onClick={onClose} aria-label="Close hotel details" className="absolute right-4 top-4 z-20 grid h-10 w-10 place-items-center rounded-full bg-[#f7f5f0]/92 text-xl">×</button>
      <div className="grid min-h-[52svh] gap-1 bg-black lg:min-h-[74svh] lg:grid-cols-3 lg:grid-rows-[2fr_1fr]">
        <PropertyImage photo={gallery} hotel={hotel} destination={destination} loading={fullMedia.loading} className="min-h-[42svh] lg:col-span-3" sizes="(min-width:1024px) 72vw,100vw" />
        {gallery.slice(1, 4).map((photo, index) => <PropertyImage key={photo.photoUri || index} photo={photo} hotel={hotel} destination={destination} className={`${index > 1 ? "hidden sm:block" : ""} min-h-[150px]`} sizes="(min-width:1024px) 24vw,50vw" />)}
      </div>
      <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-12"><h2 id="hotel-detail-title" className="pr-8 font-serif text-[clamp(2.2rem,3.6vw,3.65rem)] leading-[.92] tracking-[-.045em]">{hotel.name}</h2><p className="mt-4 text-[10px] uppercase tracking-[.13em] text-black/48">{locationLabel(hotel, destination)}</p>{ratingLabel(hotel) ? <p className="mt-4 text-xs">★ &nbsp;{ratingLabel(hotel)}</p> : null}{amenities.length ? <ul className="mt-6 grid grid-cols-2 gap-x-5 gap-y-2 border-y border-black/12 py-5 text-xs text-black/58">{amenities.map((amenity) => <li key={amenity}>{amenity}</li>)}</ul> : null}<p className="mt-6 text-sm leading-6 text-black/58">{hotel.description || `A verified stay in ${locationLabel(hotel, destination)}, selected for its fit with this trip.`}</p><p className="mt-5 text-xs text-black/48">{dateLabel(trip.tripStart) && dateLabel(trip.tripEnd) ? `${dateLabel(trip.tripStart)} — ${dateLabel(trip.tripEnd)}` : `${normalizeTravelerProfile(trip).tripLength} nights`}</p><p className="mt-6 font-serif text-2xl">{priceLabel(hotel)}</p>{bookingUrl ? <a href={bookingUrl} target="_blank" rel="noopener sponsored" onClick={() => onChoose(hotel)} className="mt-6 flex min-h-12 items-center justify-between bg-black px-6 text-[10px] uppercase tracking-[.13em] text-white">Check rooms <span aria-hidden="true">→</span></a> : <button type="button" onClick={() => { onChoose(hotel); onClose(); }} className="mt-6 flex min-h-12 items-center justify-between bg-black px-6 text-[10px] uppercase tracking-[.13em] text-white">Select this stay <span aria-hidden="true">→</span></button>}</div>
    </div>
  </div>;
}
...
function HotelCard({ hotel, destination, trip, index, count, onChoose, onDetails, onPrevious, onNext, onMedia, priority = false, proposalMode = false, selected = false, compact = false, dense = false }) {
  ...
  if (dense) return <article className="grid min-w-full snap-center gap-8 lg:grid-cols-[.86fr_1.14fr] lg:items-center lg:gap-10" aria-label={`${hotel.name}, hotel ${index + 1} of ${count}`}>
    <div className="flex min-h-[350px] flex-col justify-between py-2"><div><p className="text-[9px] uppercase tracking-[.18em]">01 &nbsp;&nbsp; Your stay</p><button type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="mt-5 block text-left"><h3 className="font-serif text-[clamp(2.1rem,3.4vw,3.45rem)] leading-[.92] tracking-[-.045em]">{verifiedHotel.name}</h3></button><p className="mt-5 text-[10px] uppercase tracking-[.12em]">{locationLabel(verifiedHotel, destination)}</p>{ratingLabel(verifiedHotel) ? <p className="mt-4 text-xs">★ &nbsp;{ratingLabel(verifiedHotel)}</p> : null}<p className="mt-5 text-xs">{profile.tripLength} nights &nbsp;·&nbsp; {dateLabel(trip.tripStart) && dateLabel(trip.tripEnd) ? `${dateLabel(trip.tripStart)} — ${dateLabel(trip.tripEnd)}` : "Dates flexible"}</p></div><div className="mt-7 flex flex-wrap items-center gap-5"><button type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="inline-flex min-h-11 items-center bg-black px-6 text-[9px] uppercase tracking-[.13em] text-white">View details &nbsp;→</button><button type="button" onClick={() => onChoose(verifiedHotel)} className="border-b border-black/40 pb-1 text-[9px] uppercase tracking-[.12em]">{selected ? "Selected ✓" : "Select stay"}</button></div></div>
    <div className="relative"><div className="relative aspect-[16/9] overflow-hidden bg-[#d8d2c8]"><PropertyImage photo={photos} hotel={verifiedHotel} destination={destination} priority={priority} loading={loading} fillContainer sizes="(min-width:1024px) 56vw,100vw" /><button type="button" onClick={() => onPrevious()} aria-label="Previous hotel" className="absolute left-3 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-[#f7f5f0]/92">←</button><button type="button" onClick={() => onNext()} aria-label="Next hotel" className="absolute right-3 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-[#f7f5f0]/92">→</button></div><div className="mt-1 grid h-20 grid-cols-3 gap-1">{photos.slice(1, 4).map((photo, photoIndex) => <button key={photo.photoUri || photoIndex} type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="relative overflow-hidden"><PropertyImage photo={photo} hotel={verifiedHotel} destination={destination} fillContainer sizes="18vw" /></button>)}</div><p className="absolute -right-1 -bottom-7 text-[9px] tracking-[.12em]">{String(index + 1).padStart(2, "0")} / {String(count).padStart(2, "0")}</p></div>
  </article>;
...
export function HotelExperience({ destination, quiz, tripContext = quiz, budgetPlan, onSelected, onSelectedMedia, proposalMode = false, selectedHotelId = null, compact = false, dense = false }) {
  ...
  </div>{drawer ? dense ? <PlannerHotelDrawer {...drawer} destination={destination} trip={tripContext} onClose={() => setDrawer(null)} onChoose={choose} /> : <HotelDrawer {...drawer} destination={destination} trip={tripContext} onClose={() => setDrawer(null)} onChoose={choose} proposalMode={proposalMode} /> : null}</section>;
}
