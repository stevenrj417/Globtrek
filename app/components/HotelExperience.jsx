...
function HotelCard({ hotel, destination, trip, index, count, onChoose, onDetails, onPrevious, onNext, priority = false, proposalMode = false, selected = false, compact = false }) {
  ...
  return <article className={`grid min-w-full snap-center bg-[#e7e0d5] transition-shadow duration-300 ${compact ? "lg:grid-cols-[.9fr_1.1fr]" : "lg:grid-cols-[.72fr_1.28fr]"} ${selected ? "shadow-[0_22px_70px_rgba(23,23,20,.12)]" : ""}`} aria-label={`${hotel.name}, hotel ${index + 1} of ${count}`}>
    <div className={`flex flex-col justify-between p-7 sm:p-10 lg:order-1 ${compact ? "lg:min-h-[480px] lg:p-10" : "lg:min-h-[650px] lg:p-12"}`}><div><div className="flex justify-between gap-4 text-[10px] text-black/45"><span>{index === 0 ? "OUR RECOMMENDATION" : "ALTERNATIVE STAY"}</span><span className="font-serif text-base text-black">{String(index + 1).padStart(2, "0")} <i className="mx-1 not-italic text-black/25">/</i> {String(count).padStart(2, "0")}</span></div><button type="button" onClick={() => onDetails({ hotel: verifiedHotel, photos })} className="mt-8 block text-left"><h3 className={`font-serif leading-[.88] tracking-[-.05em] ${compact ? "text-[clamp(2.5rem,4vw,4.2rem)]" : "text-[clamp(2.7rem,5vw,5.4rem)]"}`}>{verifiedHotel.name}</h3></button><p className="mt-6 text-sm text-black/58">{locationLabel(verifiedHotel, destination)}{verifiedHotel.starRating ? ` · ${verifiedHotel.starRating}-star` : ""} · {profile.tripLength} nights</p>{ratingLabel(verifiedHotel) ? <p className="mt-2 text-sm">{ratingLabel(verifiedHotel)}</p> : null}<p className="mt-3 text-sm font-medium">{priceLabel(verifiedHotel)}</p>{verifiedHotel.amenities?.length ? <p className="mt-8 text-sm leading-7 text-black/55">{verifiedHotel.amenities.slice(0, compact ? 3 : 5).join(" · ")}</p> : null}</div>
...
export function HotelExperience({ destination, quiz, budgetPlan, onSelected, proposalMode = false, selectedHotelId = null, compact = false }) {
  ...
  </div>{drawer ? <HotelDrawer {...drawer} destination={destination} trip={quiz} onClose={() => setDrawer(null)} onChoose={choose} proposalMode={proposalMode} /> : null}</section>;
}
