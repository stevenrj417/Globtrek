...
function HotelCard({ hotel, destination, trip, index, count, onChoose, onDetails, onPrevious, onNext, priority = false, proposalMode = false, selected = false }) {
  ...
  return <article className={`grid min-w-full snap-center bg-[#e7e0d5] transition-shadow duration-300 lg:grid-cols-[.72fr_1.28fr] ${selected ? "shadow-[0_22px_70px_rgba(23,23,20,.12)]" : ""}`} aria-label={`${hotel.name}, hotel ${index + 1} of ${count}`}>
...
export function HotelExperience({ destination, quiz, budgetPlan, onSelected, proposalMode = false, selectedHotelId = null }) {
