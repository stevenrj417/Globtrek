...
export function AuthSheet({ open, onClose, returnTo = "/" }) {
  ...
  return <div className="fixed inset-0 z-[100] bg-black/25 backdrop-blur-[2px]" role="presentation" onMouseDown={close}>
    <section role="dialog" aria-modal="true" aria-labelledby="auth-title" className="absolute inset-x-0 bottom-0 bg-[#f7f7f4] px-6 pb-10 pt-6 shadow-[0_-20px_80px_rgba(0,0,0,.12)] sm:left-auto sm:right-6 sm:bottom-6 sm:w-[430px] sm:px-10 sm:pb-10 sm:pt-8" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {!emailMode ? <div className="mt-8 grid gap-3">
        <button type="button" onClick={() => continueWith("apple")} className="min-h-14 border border-black bg-black px-5 text-[10px] uppercase tracking-[0.18em] text-white">Continue with Apple</button>
        <button type="button" onClick={() => continueWith("google")} className="min-h-14 border border-black/20 bg-white px-5 text-[10px] uppercase tracking-[0.18em]">Continue with Google</button>
