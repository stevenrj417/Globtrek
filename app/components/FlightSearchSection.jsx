...
export function FlightSearchSection({ destination, trip = {} }) {
  ...
  return <section className="mt-20" aria-labelledby="flight-search-heading">
    <h2 id="flight-search-heading" className="border-b border-black/20 pb-6 font-serif text-[clamp(2.2rem,4vw,3.8rem)] tracking-[-0.04em]">Flights</h2>
    <div className="border-x border-b border-black/15 bg-[#f8f6f2] p-5 sm:p-7">
      <div className="grid border-l border-t border-black/15 sm:grid-cols-2 lg:grid-cols-[.8fr_.8fr_1fr_1fr_.6fr]">
        <label className="border-b border-r border-black/15 p-4"><span className="text-[8px] uppercase tracking-[0.18em] text-black/70">From</span><input required aria-label="Origin airport" value={origin} maxLength={3} placeholder="PDX" onChange={(event) => change(setOrigin)(event.target.value.toUpperCase().replace(/[^A-Z]/g, ""))} className="mt-2 block w-full bg-transparent font-serif text-xl uppercase text-black outline-none" /></label>
        <div className="border-b border-r border-black/15 p-4"><span className="text-[8px] uppercase tracking-[0.18em] text-black/70">To</span><strong className="mt-2 block font-serif text-xl font-normal text-black">{destination.airport}</strong></div>
        <label className="border-b border-r border-black/15 p-4"><span className="text-[8px] uppercase tracking-[0.18em] text-black/70">Depart</span><input type="date" value={depart} onChange={(event) => change(setDepart)(event.target.value)} className="mt-2 block w-full bg-transparent text-sm text-black outline-none" /></label>
        <label className="border-b border-r border-black/15 p-4"><span className="text-[8px] uppercase tracking-[0.18em] text-black/70">Return</span><input type="date" min={depart || undefined} value={returnDate} onChange={(event) => change(setReturnDate)(event.target.value)} className="mt-2 block w-full bg-transparent text-sm text-black outline-none" /></label>
        <label className="border-b border-r border-black/15 p-4"><span className="text-[8px] uppercase tracking-[0.18em] text-black/70">Adults</span><input type="number" min="1" max="30" value={adults} onChange={(event) => change(setAdults)(event.target.value)} className="mt-2 block w-full bg-transparent text-sm text-black outline-none" /></label>
      </div>
      ...
      <div className="mt-5 flex justify-end border-t border-black/15 pt-5">
