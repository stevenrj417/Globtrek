...
export function tripEmail({ model, viewUrl }) {
  ...
  const selectionRow = (item, link) => item ? `<div style="border-top:1px solid #d8d4ce;padding:14px 0"><span style="font-size:13px">${escapeHtml(item.name)}</span>${link ? ` <a href="${escapeHtml(link)}" style="float:right;color:#171714;font-size:11px">Booking details →</a>` : `<span style="float:right;color:#777;font-size:11px">Link being verified</span>`}</div>` : "";
  const restaurantLinks = new Map((model.bookingLinks?.restaurants || []).map((item) => [item.name, item.url]));
  const activityLinks = new Map((model.bookingLinks?.activities || []).map((item) => [item.name, item.url]));
  const selections = [selectionRow(model.hotel, model.bookingLinks?.hotel), ...(model.restaurants || []).map((item) => selectionRow(item, restaurantLinks.get(item.name))), ...(model.activities || []).map((item) => selectionRow(item, activityLinks.get(item.name)))].join("");
  const logistics = `<p style="font-size:12px;line-height:1.8;color:#555">${model.dates?.start ? `${escapeHtml(model.dates.start)}–${escapeHtml(model.dates.end || "")}` : "Flexible dates"}${model.travelers ? ` · ${escapeHtml(model.travelers)} travelers` : ""}${model.exactBudget ? ` · ${money(model.exactBudget)} budget` : ""}</p>`;
  const selectionHtml = selections ? `<div style="margin-top:38px"><p style="font-size:9px;letter-spacing:.2em;text-transform:uppercase;color:#777">Your selections</p>${selections}</div>` : "";
  return shell({ preheader: `Your complete ${days.length}-day Globtrek itinerary.`, eyebrow: "Your trip, organized", title: `${days.length} days in ${model.destination?.city || model.destination?.name || "your destination"}`, body: `${logistics}${costHtml}${selectionHtml}${viewUrl ? button("View trip", viewUrl) : ""}<div style="margin-top:42px">${dayHtml}</div>`, footer: "This transactional trip email does not subscribe you to marketing. Confirm final prices and availability with each provider." });
}
