...
export function tripEmail({ model, viewUrl }) {
  ...
  return shell({ preheader, eyebrow: "Your trip, organized", title, body: `${logistics}${costHtml}${journeyHtml}${selectionHtml}${viewUrl ? button("View trip", viewUrl) : ""}<div style="margin-top:42px">${dayHtml}</div>`, footer: "This transactional trip email does not subscribe you to marketing. Confirm final prices and availability with each provider." });
}
