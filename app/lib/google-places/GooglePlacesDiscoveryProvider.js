...
export class GooglePlacesDiscoveryProvider extends GooglePlacesHotelProvider {
  ...
  async discoverActivities(destination, { limit = 12 } = {}) {
    ...
    const records = await Promise.all(places.map(async (place, index) => ({ id: `google:${place.id}`, destinationId: destination.id || destination.airport, name: placeName(place), description: null, category: category(place, "local"), location: place.formattedAddress || null, latitude: place.location?.latitude ?? null, longitude: place.location?.longitude ?? null, provider: "google_places", providerId: place.id, googlePlaceId: place.id, bookingUrl: null, officialWebsiteUrl: place.websiteUri || null, websiteUri: place.websiteUri || null, googleMapsUrl: place.googleMapsUri || null, googleMapsUri: place.googleMapsUri || null, detailsUrl: place.googleMapsUri || null, rating: Number(place.rating) || null, reviewCount: Number(place.userRatingCount) || null, ...(index < 6 ? await this.firstPhoto(place) : { imageUrl: null, imageAttribution: [], imageSourceUrl: place.googleMapsUri || null }), verifiedAt: new Date().toISOString() })));
    return remember(key, records).slice(0, limit);
  }
