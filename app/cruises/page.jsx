...
export default function CruisesPage() {
  return <main className="min-h-screen bg-[#f7f7f4] text-[#171714]">
    ...
    <section className="px-3 pb-3 pt-3 sm:px-5 sm:pb-5"><div className="relative mx-auto min-h-[calc(100svh-7.25rem)] max-w-[1880px] overflow-hidden bg-[#728c99]"><ExactPlacePhoto placeId="ChIJeTCXLQ90BYwRvPklLQww53Q" alt="The Caribbean Sea" priority className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-b from-black/10 via-black/5 to-black/70" /><div className="absolute inset-x-0 bottom-0 px-6 pb-10 text-white sm:px-12 sm:pb-14 lg:px-16 lg:pb-16"><p className="text-[10px] uppercase tracking-[0.24em] text-white/75">GlobTrek Cruises</p><div className="mt-5 flex flex-col items-start justify-between gap-9 md:flex-row md:items-end"><h1 className="max-w-6xl font-serif text-[clamp(4rem,9.4vw,9.5rem)] font-normal leading-[.8] tracking-[-0.065em]">Find the ocean journey<br className="hidden md:block" /> worth taking.</h1><Link href="/cruises/quiz" className="inline-flex min-h-14 shrink-0 items-center justify-between gap-12 bg-white px-6 text-xs font-semibold uppercase tracking-[0.09em] text-black transition hover:bg-[#eee9df]">Create my cruise <span aria-hidden="true">→</span></Link></div></div></div></section>
    ...
    <JourneyCards items={cruiseDiscovery} basePath="/cruises/quiz" eyebrow="Trending cruise journeys" />
    <SiteFooter />
  </main>;
}
