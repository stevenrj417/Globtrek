...
export default function FullTripPage({ params }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const response = await fetch(id.startsWith("s_") ? `/api/public-trips/${encodeURIComponent(id)}` : `/api/trips/${encodeURIComponent(id)}`);
  ...
  return <main className="min-h-screen bg-[#f3f0eb] text-[#171714]">
    ...
    <section className="mx-auto max-w-[1460px] px-6 pb-24 sm:px-12">
      <div className="grid gap-px bg-black/10 sm:grid-cols-[1fr_auto_auto_auto_auto]"><div className="bg-[#f3f0eb] px-5 py-5"><p className="text-[8px] uppercase tracking-[0.2em] text-black/40">Complete itinerary</p><p className="mt-2 font-serif text-xl">Every day, one shared plan.</p></div>{!id.startsWith("s_") ? <SaveTripButton trip={savedTrip} className="min-h-20 bg-[#f3f0eb] px-7 text-[9px] uppercase tracking-[0.2em] hover:bg-white" /> : null}<EmailTripButton trip={savedTrip} viewUrl={`/trip/${id}`} className="min-h-20 bg-[#f3f0eb] px-7 text-[9px] uppercase tracking-[0.2em] hover:bg-white" />{id !== "current" && !id.startsWith("s_") ? <ShareTripButton tripId={id} className="min-h-20 bg-[#f3f0eb] px-7 text-[9px] uppercase tracking-[0.2em] hover:bg-white" /> : null}<Link href="/results" className="grid min-h-20 place-items-center bg-[#f3f0eb] px-7 text-[9px] uppercase tracking-[0.2em] hover:bg-white">Back to overview</Link></div>
      <ItineraryDocument trip={trip} quiz={quiz} previewDays={trip.plan.days.length} />
    </section>
  </main>;
}
