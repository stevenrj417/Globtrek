...
function publicDestination(destination) {
  return {
    ...
    country: destination.country, latitude: destination.latitude, longitude: destination.longitude, image: destination.image, style: destination.style,
  };
}

export async function POST(request) {
  const answers = await request.json().catch(() => null);
  if (!answers || Number(answers.budget) < 500) return Response.json({ error: "Incomplete road-trip profile" }, { status: 400 });
  ...
  const route = {
    ...
    hotelDestinationId: selected.hotelDestinationId, estimate: selected.estimate, compatibility: selected.compatibility,
